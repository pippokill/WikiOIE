 ...
 * @author Alessia
 */
public class TripleFullVectorSim {
    ...
    public TripleFullVectorSim(String indexDir, String vectorIndexDir, STORE_TYPE type) {
    ...
    public List<Counter> discoverSimilPred(String pred, VectorReader vr, int n, double cosine_threshold) throws IOException, ParseException {
        ...
        for (TripleVectorResult r : predTriples) {
            ...
            Vector subjVector = getVectorFromText(t.getSub(), vr);
            Vector objVector = getVectorFromText(t.getObj(), vr);
            List<TripleVectorResult> subjSim = vectorIndex.findSimilarSubject(subjVector);
            Set<Integer> tids = new HashSet<>();
            subjSim.stream().forEach(x -> tids.add(x.getDocid()));
            List<TripleVectorResult> objSim = vectorIndex.findSimilarObject(objVector);
            Set<Integer> oids = new HashSet<>();
            objSim.stream().forEach(x -> oids.add(x.getDocid()));
            tids.retainAll(oids);
            for (Integer docid : tids) {
    ...
    public List<Counter> discoverSimilSubj(String subj, VectorReader vr, int n, double cosine_threshold) throws IOException, ParseException {
        ...
        for (TripleVectorResult r : subjTriples) {
            Triple t = getTriple(r.getDocid());
            Vector predVector = getVectorFromText(t.getPred(), vr);
            Vector objVector = getVectorFromText(t.getObj(), vr);
            List<TripleVectorResult> predSim = vectorIndex.findSimilarPredicate(predVector);
            Set<Integer> tids = new HashSet<>();
            predSim.stream().forEach(x -> tids.add(x.getDocid()));
            List<TripleVectorResult> objSim = vectorIndex.findSimilarObject(objVector);
            Set<Integer> oids = new HashSet<>();
            objSim.stream().forEach(x -> oids.add(x.getDocid()));
            tids.retainAll(oids);
    ...
    public List<Counter> discoverSimilObj(String obj, VectorReader vr, int n, double cosine_threshold) throws IOException, ParseException {
        ...
        for (TripleVectorResult r : objTriples) {
            ...
            predSim.stream().forEach(x -> pids.add(x.getDocid()));
            tids.retainAll(pids);
            for (int docid : pids) {
