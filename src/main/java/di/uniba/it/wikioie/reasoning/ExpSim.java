...
public class ExpSim {
    ...
    public static void main(String[] args) {
        try {
            ...
            Appendable out = new FileWriter("./preds_sim.csv");
            CSVPrinter printer = CSVFormat.DEFAULT.print(out);
            for (String q : preds) {
                printer.printRecord(q, "", "");
                List<Counter> l = sim.discoverSimilPred(q, vr, n, cosine_threshold);
            ...
            out = new FileWriter("./subjs_sim.csv");
            printer = CSVFormat.DEFAULT.print(out);
            for (String q : subjs) {
                printer.printRecord(q, "", "");
                List<Counter> l = sim.discoverSimilSubj(q, vr, n, cosine_threshold);
                for (int i = 0; i < l.size() && i < topn; i++) {
                    printer.printRecord("", l.get(i).getItem().toString(), l.get(i).getCount());
                }
                printer.flush();
            ...
            out = new FileWriter("./objs_sim.csv");
            printer = CSVFormat.DEFAULT.print(out);
            for (String q : objs) {
                printer.printRecord(q, "", "");
                List<Counter> l = sim.discoverSimilObj(q, vr, n, cosine_threshold);
                for (int i = 0; i < l.size() && i < topn; i++) {
                    printer.printRecord("", l.get(i).getItem().toString(), l.get(i).getCount());
                }
                printer.flush();
            ...
            Logger.getLogger(ExpSim.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
